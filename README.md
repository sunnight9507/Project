...
### 시스템 구성도
<div>
  <img width="1000" src="https://github.com/Bigjob-team-12/Project/blob/master/_img/system.png">
</div>
...
- **주요 기능**
  - 1. **특정 개체 특성 중심 탐색**
    - > 품종 등 메타데이터를 바탕으로 한 단순 검색을 넘어 개체의 시각적 특성을 바탕으로 한 유사성 비교를 수행
    - 공고 데이터의 90%를 차지하는 믹스견 역시 세부적인 특징을 분석해 유사한 개체를 찾아냄
  - 2. **파이프라인 자동화**
    - 주기적으로 신규 등록 개체 수집 및 분석하여 기존 Query 개체와 비교
    - 유사도가 높은 개체 발견될 경우 메일 발송
  - 3. **Fine-Grained Classification**
    - 세부적인 품종의 외견 정보를 바탕으로 분류를 수행
    - 여러 품종의 특성을 가진 믹스견을 고려해, 외형적으로 추측되는 각 품종의 확률값을 도출
  - 4. **Metric Learning**
    - Query 이미지와 유사한 색상(단일 컬러 및 여러 색의 혼합)인 개체를 찾기 위해 특성을 추출한 후, Similarity를 계산
    - Similarity 계산을 위해 Metric Learning을 적용
  - 5. **Stacking ensemble**
    - Fine-Grained Classification으로 각 품종 확률값 기준 필터링
    - 필터링된 이미지를 Metric Learning을 통해 털의 색, 조합의 유사성 확인
